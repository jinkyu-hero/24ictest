...
if uploaded_file is not None:
    ...
    # 3. 반 선택 후 각 과목 점수 산점도 (가로 세로를 바꾸고 한 차트에 표시)
    st.subheader(f'반별 점수 산점도 (한 차트에 표시)')
    ...
    # 한 차트에 모든 과목의 점수 산점도를 그리기
    scatter_chart = alt.Chart(filtered_data).mark_point(filled=True).encode(
        y=alt.Y('반:O', title='반'),
        x=alt.X(alt.repeat("row"), type='quantitative'),
        color='과목:N',
        tooltip=['반', alt.repeat("row")]
    ).repeat(
        row=['국어', '수학', '영어', '한국사', '탐구1_점수', '탐구2_점수'],
        title=f"{selected_class} 반의 점수 분포"
